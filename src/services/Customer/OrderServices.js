  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      for (const item of productItems) {
        ...
        if (!shop)
          throw new Error(`Shop not found for product: ${item.productName}`);
        ...
        if (!matchedOption) {
          throw new Error(
            `Không tìm thấy biến thể phù hợp cho sản phẩm: ${item.productName}`
          );
        }

        if (matchedOption.stock < item.quantity) {
          throw new Error(
            `Sản phẩm "${item.productName}" không đủ tồn kho. Còn lại: ${matchedOption.stock}, bạn chọn: ${item.quantity}`
          );
        }
          ...
          cart.productItems = cart.productItems.filter((cartItem) => {
            return !productItems.some(
              (orderItem) =>
                ...
                JSON.stringify(orderItem.attributes) ===
      ...
      for (const voucher of vouchers) {
        const foundVoucher = await Voucher.findById(voucher.voucherId).session(
          session
        );
        if (foundVoucher) {
      ...
      const calculateFinalShippingFeeByShop = (shippingFeeByShop, vouchers) => {
        ...
        const shippingVouchers = vouchers.filter(
          (v) => v.category === "van-chuyen"
        );

        // Tính tổng phí ship ban đầu
        const totalShippingFee = Object.values(shippingFeeByShop).reduce(
          (sum, fee) => sum + fee,
          0
        );
        ...
        for (const voucher of shippingVouchers) {
          if (voucher.type === "fixed") {
            discountedTotalFee = Math.max(
              discountedTotalFee - voucher.value,
              0
            );
          } else if (voucher.type === "percentage") {
            discountedTotalFee = Math.max(
              discountedTotalFee * (1 - voucher.value / 100),
              0
            );
          }
        }
        ...
        const sumFinalFee = result.reduce(
          (sum, item) => sum + item.shippingFeeFinal,
          0
        );
      ...
      const shippingByShopArray = calculateFinalShippingFeeByShop(
        shippingFeeByShop,
        vouchers
      );
...
const successfulDelivered = async (user_id, { order, status }) => {
  try {
    ...
    const successDelivered = await Order.findByIdAndUpdate(
      order_id,
      {
        status: status,
      },
...
const canceledOrder = async (
  user_id,
  { order, productList, shippingByShopList, cancelReason, voucherList }
) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const shopIdsToCancel = [
        ...new Set(productList.map((p) => p.shopId.toString())),
      ];
      ...
      const allCancelled = orderBill.productItems.every(
      ...
      if (paymentMethod === "Online" || paymentMethod === "Wallet") {
        ...
        const shopShipping = shippingByShopList.find(
          (s) => s.shopId.toString() === shopId
        );
        const shippingFeeShop = shopShipping
          ? shopShipping.shippingFeeFinal || 0
          : 0;
        ...
        const totalVoucherDiscountOnShipping = voucherList
          .filter(
            (v) =>
              v.category === "van-chuyen" &&
              typeof v.discountAmount === "number"
          )
        ...
        const ratioProduct =
          totalProductPriceAll > 0
            ? totalProductPriceShop / totalProductPriceAll
            : 0;
        const ratioShipping =
          totalShippingFeeAll > 0 ? shippingFeeShop / totalShippingFeeAll : 0;

        const voucherDiscountOnProducts =
          totalVoucherDiscountOnProducts * ratioProduct;
        const voucherDiscountOnShipping =
          totalVoucherDiscountOnShipping * ratioShipping;

        const amountPaid =
          typeof orderBill.finalAmount === "number" && orderBill.finalAmount > 0
            ? orderBill.finalAmount
            : orderBill.totalPrice -
              (totalVoucherDiscountOnProducts + totalVoucherDiscountOnShipping);

        const refundAmount =
          amountPaid * ratioProduct +
          shippingFeeShop -
          voucherDiscountOnShipping;
...
const returnOrder = async (
  user_id,
  { order, productList, shippingByShopList, refundReason, voucherList = [] }
) => {
  return new Promise(async (resolve, reject) => {
    try {
      const orderBill = await Order.findById(order._id);
      ...
      const shopIdsToReturn = [
        ...new Set(productList.map((p) => p.shopId.toString())),
      ];
      if (shopIdsToReturn.length > 1) {
        return reject({
          message: "Chỉ được hoàn trả sản phẩm của 1 shop mỗi lần",
        });
      }
      ...
      if (invalidItem) {
        return reject({
          message: "Một số sản phẩm đã xử lý, không thể hoàn trả",
        });
      }
      ...
      if (!shop) return reject({ message: "Shop không tồn tại" });

      const owner = await User.findById(shop.ownerId);
      if (!owner) return reject({ message: "Không tồn tại chủ shop" });
      ...
      // Trừ tiền owner
      owner.wallet = (owner.wallet || 0) - totalReturnAmount;
      await owner.save();
      ...
      const totalVoucherDiscountOnShipping = voucherList
        .filter(
          (v) =>
            v.category === "van-chuyen" && typeof v.discountAmount === "number"
        )
        .reduce((sum, v) => sum + v.discountAmount, 0);
      ...
      const refundAmount =
        amountPaid * ratioProduct + shippingFeeShop - voucherDiscountOnShipping;
      ...
      resolve({
        message:
          "Đã hoàn trả sản phẩm của shop thành công, tiền đã được cộng vào ví người dùng.",
        order: orderBill,
        refundAmount,
      });
    } catch (error) {
